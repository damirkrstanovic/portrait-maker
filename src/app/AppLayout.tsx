...
export function AppLayout({ library, busy, onImport, onExport, onDestinations, destinationsTriggerRef, onBackup, onFindDuplicates, onAnalyze, onCloseLibrary, children }: Props) {
      ...
      <div className="library-actions">
        ...
        <div className="library-menu" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setMenu(false); }} onKeyDown={event => { if (event.key === "Escape") { setMenu(false); menuTrigger.current?.focus(); } }}>
          ...
          {menu && <div className="library-menu-actions" id="library-menu-actions">
            {onAnalyze && <button type="button" onClick={() => { setMenu(false); onAnalyze(); }}>Describe portraits</button>}
            {onFindDuplicates && <button type="button" onClick={() => { setMenu(false); onFindDuplicates(); }}>Find duplicates</button>}
